...
export interface JsonListTableProps {
  rows: Record<string, unknown>[];
  onCopyTable?: (tableText: string) => void;
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {Data.map((row, idx) => (
          ...
          >
            <header className="border-b border-border px-4 py-3 ui-card-header-muted shadow-sm">
              <h3 className="text-base font-semibold text-card-foreground font-app">
              ...
              <dl className="grid grid-cols-1 gap-2.5 text-sm sm:grid-cols-2">
                {DETAIL_FIELDS.map(({ key, label }) => {
                  ...
                  return (
                    ...
                    >
                      <dt className="mb-0.5 text-xs font-medium uppercase tracking-wide text-muted-foreground font-app">
                        {label}
                      </dt>
                      <dd className="text-card-foreground font-app">
                        {isPill && text !== '—' ? (
                          <span
...
function toTabularText(rows: Record<string, unknown>[], columns: string[]): string {
  const header = columns.map((key) => formatKeyLabel(key)).join('\t');
  const body = rows
    .map((row) => columns.map((key) => formatCellValue(row[key])).join('\t'))
    .join('\n');
  return `${header}\n${body}`.trim();
}

export function JsonListTable({ rows, onCopyTable }: JsonListTableProps) {
  if (rows.length === 0) return null;

  const columns = tableColumns(rows);
  const copyText = toTabularText(rows, columns);

  return (
    <div className="min-w-0 max-w-full">
      {/* ChatGPT: toolbar row — record hint + copy icon top-right of table area */}
      <div className="relative mb-1 flex min-w-0 items-center justify-between gap-3">
        <p className="font-app text-xs text-neutral-500 dark:text-neutral-400">
            ...
            <TooltipTrigger asChild>
              <button
                ...
                className={cn(
                  ...
                  'border border-neutral-200 bg-transparent text-neutral-400 transition-colors',
                  'hover:bg-neutral-100 hover:text-neutral-600',
                  'dark:border-neutral-700 dark:text-neutral-500 dark:hover:bg-neutral-800 dark:hover:text-neutral-300'
                )}
                onClick={() => onCopyTable(copyText)}
                aria-label="Copy table"
              >
                <Copy size={18} strokeWidth={1.75} className="shrink-0" />
              </button>
            </TooltipTrigger>
            <TooltipContent
              side="bottom"
              sideOffset={8}
              className={cn(
                'rounded-full border-0 bg-neutral-950 px-3 py-1.5 text-xs font-medium text-white shadow-lg',
                'dark:bg-neutral-950 [&>svg]:hidden'
              )}
            >
              Copy table
            </TooltipContent>
      ...
      {/* ChatGPT: no outer box — same surface as message; horizontal rules only */}
      <div className="chat-json-table-scroll ui-themed-scroll min-w-0">
        <table
          className={cn(
            'w-max min-w-full border-collapse bg-transparent text-left font-app text-sm antialiased',
        ...
        >
          <thead>
            <tr className="border-b border-neutral-200 dark:border-neutral-800">
              {columns.map((key) => (
                <th
                  key={key}
                  className="whitespace-nowrap px-2 py-4 text-left text-sm font-semibold text-neutral-900 first:pl-0 last:pr-0 dark:text-neutral-100 sm:px-3"
                >
                  {formatKeyLabel(key)}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr
                key={stableRowKey(row, rowIndex)}
                className="border-b border-neutral-200 last:border-b-0 dark:border-neutral-800"
              >
                {columns.map((key) => (
                  <td
                    key={`${rowIndex}-${key}`}
                    className="min-w-0 align-top whitespace-normal break-words px-2 py-4 text-sm font-normal leading-relaxed text-neutral-800 first:pl-0 last:pr-0 dark:text-neutral-200 sm:px-3"
                  >
